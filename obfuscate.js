...
async function obfuscateWithPrometheus(inputCode, preset = 'Medium') {
  ...
  try {
    ...
    // Command to run original Prometheus CLI (inside Docker)
    // The Prometheus repo is cloned to /prometheus
    const command = `lua /prometheus/cli.lua ${inputFile} --preset ${preset} --output ${outputFile}`;
